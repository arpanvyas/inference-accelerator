...
def prog_conv(model_map,model_idx_start,interm_map,interm_idx_start,all_layers,layer_index,regfile,dump_mem_instr_log,next_flatten):
    ...
    #        print("expected: "+"input_layer"+str(layer_idx)+"_ch"+str(ch) +" found:"+interm_map[interm_idx][2])
    #        return
    ...
    #            ram_number  = interm_map[interm_idx][1]
    #        else:
    #            print("program.py: Output parsing error in layer conv")
    #            print("expected: "+"input_layer"+str(layer_idx)+"_ch"+str(ch) +" found:"+interm_map[interm_idx][2])
    #            return
    ...
    if(next_flatten == 1): 
        ...
        for buf_loop in range(0,mem.buffer_num):
            ...
            ram_number = 0
            any_output_at_all = 0
            while(ch < channels_next):
                if(interm_map[interm_idx][2] == "input_layer"+str(layer_idx+1)+"_ch"+str(ch)):
                    if(ch == buf_loop):
                        ram_start   = interm_map[interm_idx][0]
                    ram_number  += interm_map[interm_idx][1]
                else:
                    print("program.py: Saving Output parsing error in layer conv")
...
def prog_dense(model_map,model_idx_start,interm_map,interm_idx_start, all_layers, layer_index, regfile, dump_mem_instr_log):
    ...
    ##BELOW FUNCTION TO BE OPTIMIZED
    ...
    ##OPTIMIZED
    ##collect all the data to be written before loading it
    ram_number = 0
    buffer_addr = mem.buffer_num #input (i.e. vector V stored in buffer 32)
    for ipnu in range(0,input_nodes):
        if(interm_map[interm_idx][2] == "input_layer"+str(layer_idx)+"_nod"+str(ipnu)):
            if(ipnu == 0):
                ram_start = interm_map[interm_idx][0]
            ram_number += interm_map[interm_idx][1]
            #print("Good: ipnu is ", ipnu)
        else:
            print("program.py: Input parsing error in layer dense")
            print(interm_map[interm_idx][2] + " != " + "input_layer"+str(layer_idx)+"_nod"+str(ipnu))
            #print("interm_idx_start is: ", interm_idx_start)
            #print("interm_idx is: ", interm_idx)
            #print("Bad: ipnu is ", ipnu)
            return
        interm_idx += 1
    ...
    #OPTIMIZED
    ##collect all the data to be written before saving it
    ram_number = 0
    buffer_addr = mem.buffer_num #because dense output stored in buffer#32
    for node in range(0,nodes_next):
