...
for layer in conf['layers']:
    ...
    if(layer_type == "Conv2D"):
        ...
        mem_ptr = dump.dump_conv(dumpfile,mem_map,mem_ptr,this_layer)
        ...
        mem_ptr = dump.dump_dense(dumpfile,mem_map,mem_ptr,this_layer)
...
#print(*conf['layers'][0]['config']['data_format'],sep="")
#print(*conf['layers'][4]['config'],sep="\n")
#print(conf['layers'][4]['config'])


print(*next_layer_all,sep="\n")

mem_map_file = open("mem_map","w")
for item in mem_map:
    mem_map_file.write("%s\n" % item)
